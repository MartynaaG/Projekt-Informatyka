...
from copy import deepcopy#to jest kopia gleboka, bo jak pisalem te if-y do poprawnosci true i false to w ogole mi nie wchodzilo w te warunki moje(wydaje mi sie ze to przez to, ze psychopy ma jakis problem z tym przepisaniem do previous_go i  Go/ NoGo)
...
def main():
    ...
    logging.info('FRAME RATE: {}'.format(conf['FRAME_RATE'])) #zbierze info o klatkach i rozdzielczosci, ale nw czy to nie przysporzy nam jakiegos bledu, ustawmy w configu odgornie i bedzie procedura dopasowana do konkretnego komputera z mozliwoscia zmienienia w configu
